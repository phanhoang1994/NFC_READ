...
void loop(void) {
  ...
  switch (ModeWork) {
    case SET: {
        switch (fun_nfc.cardNFCMaster.mode) {
          ...
          case DELETE: {
              switch (fun_nfc.cardNFCMaster.modeD) {
                case 1: {
                    if (fun_nfc.DeleteDataEeprom(fun_nfc.AddrEepromDel, SIZE))
                    {
                      fun_nfc.cardNFCMaster.modeD = 0;
                    }
                    ulTimeLock = 30;
                    break;
                  }
                case 2: {
                    if (fun_nfc.DeleteDataEeprom(fun_nfc.AddrEepromDel, (fun_nfc.cardNFCMaster.numdel)*SIZE)){
                      fun_nfc.cardNFCMaster.modeD = 0;
                    }
                    ulTimeLock = 30;
                    break;
                ...
                case 3: {
                    if (fun_nfc.have_new_user == 1) {
                      if (fun_nfc.DeleteDataEeprom(fun_nfc.AddrEeprom, SIZE)) {
                        beep(100); 
                      }
                      ulTimeLock = 30;
                      fun_nfc.have_new_user = 0;
        ...
        //Nếu trong 30s trong chế độ SET không có thẻ nào được quẹt thì tự động khoá
        if (ulTimeLock == 0) {
          ModeWork = READ;
          fun_nfc.have_new_master = 0;
          ucRelay = 0;
